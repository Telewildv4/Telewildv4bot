...
local function instagramUser(msg, query)
 ...
 if user.data.bio ~= '' then
  text = text.." 馃啍鈾愶笍  "..user.data.username:upper().."\n\n"
 else
  text = text.."馃啍鈾愶笍  "..user.data.username:upper().."\n"
 end
 if user.data.bio ~= '' then
 ...
 text = text.."馃懁馃摲 "..user.data.counts.media.."\n"
 text = text.."馃懃鉃★笍 "..user.data.counts.follows.."\n"
 text = text.."馃懃猬咃笍 "..user.data.counts.followed_by.."\n"
